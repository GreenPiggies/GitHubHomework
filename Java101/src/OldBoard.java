...
public class OldBoard 
{
	...
	{
		for (int row = 0; row < boardSize; row++)//deletes all places blocked by that queen
		{
			for (int column = 0; column < boardSize; column++)
			{
	...
	public void queenBlock(int queenRow, int queenColumn)
	{
	...
	{
		StdDrawPlus.picture(x + .5, boardSize - 1 - y + .5, "queen-dark.png", 1, 1);
	}
	public void findAllQueens()
	{
		...
		while (true)
		{
			...
			{				
				...
				if (checkSpaces)//this is for marking
				{
					queenRow = numQueens; 
					queenColumn = columns;
					this.queenBlock(queenRow, queenColumn);
					numQueens++;		
				}
				else//deletes queen stuff
				{
					if(numQueens <= 1)
					{
						break outerloop;
					...
					if (checkPositions == 0)//when there is no place to put the queen on that row
					{
						...
						this.delete(1);//deletes invalid areas made by the previous queen
						this.marker(2);//marks it as invalid. 
						numQueens--;
					}
					numQueens--;
				}
				if (numQueens == boardSize)
				{
					...
					int[] holdQueens = new int[boardSize];
					for (int row = 0; row < boardSize; row++)
					{
						for (int column = 0; column < boardSize; column++)
						{
					...
					for(int row = 0; row < boardSize; row++)
					{
						for(int column = 0; column < boardSize; column++)
						{
							if(holdQueens[row] == column)
							{
								board[row][column] = 1;
								this.queenBlock(row, column);
								numQueens++;
							}
						}
					}
